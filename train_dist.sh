...
if [ $# -lt 3 ]
then
    echo "Usage: bash $0 LOG_OUT DATA GPUS"
    exit
fi
...
if [ ${GPUS} == 1 ]
then
    nohup python train_global_criss_D2Tcross_dist.py -b 8 --lr 6e-4 --dataset $DATA 2>&1 >> $LOG_OUT &
else
    ...
    -b 8 --lr 6e-4 --dataset $DATA 2>&1 >> $LOG_OUT &
fi