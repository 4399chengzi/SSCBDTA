...
class MolTransModel(nn.Sequential):
    ...
    def __init__(self, model_config):
        ...
        # self.T2D_cross_attn = Cross_Attention(self.hidden_size, self.num_attention_heads, 
        #                               self.attention_dropout_ratio, self.hidden_dropout_ratio, mode='T2D')
        
        self.D2T_cross_attn = Cross_Attention(self.hidden_size, self.num_attention_heads, 
                                      self.attention_dropout_ratio, self.hidden_dropout_ratio, mode='D2T')
        ...
        # self.interaction_cnn = CrissCrossAttention(self.emb_size)
    ...
    def forward(self, d, t, d_masking, t_masking,
        ...
        print("tempd_masking:", tempd_masking.shape, tempd_masking[0])
        print("tempt_masking:", tempt_masking.shape, tempt_masking[0])
        ...
        print("d_embedding:", d_embedding.shape, d_embedding[0])
        print("t_embedding:", t_embedding.shape, t_embedding[0])
        ...
        print("complete_d_embedding:", complete_d_embedding.shape, complete_d_embedding[0])
        print("complete_t_embedding:", complete_t_embedding.shape, complete_t_embedding[0])
        ...
        transformer module
        """
        d_embedding = d_embedding + complete_d_embedding #将complete_d_embedding广播到d_embedding的维度
        t_embedding = t_embedding + complete_t_embedding #将complete_d_embedding广播到d_embedding的维度
        print("d_embedding:", d_embedding.shape, d_embedding[0])
        print("t_embedding:", t_embedding.shape, t_embedding[0])
        ...
        print("d_encoder:", d_encoder.shape, d_encoder[0])
        print("t_encoder:", t_encoder.shape, t_encoder[0])


        # crs_d_encoder = self.T2D_cross_attn(d_encoder, t_encoder, tempt_masking.float())
        crs_t_encoder = self.D2T_cross_attn(d_encoder, t_encoder, tempd_masking.float())
        # print("crs_t_encoder:", crs_t_encoder.shape, crs_t_encoder[0])


        drug_res = torch.unsqueeze(d_encoder, 2).repeat(1, 1, self.target_max_seq, 1) # N, len(drug),len(protein), emb_dim
        target_res = torch.unsqueeze(crs_t_encoder, 1).repeat(1, self.drug_max_seq, 1, 1) # N, len(drug),len(protein), emb_dim
        # print("drug_res:", drug_res.shape, drug_res[0])
        # print("target_res:", target_res.shape, target_res[0])
        ...
        # print("i_scoreT:", i_scoreT.shape, i_scoreT[0])
        ...
        # print("i_res:", i_res.shape, i_res[0])
        # torch.Size([32, 872000])

        res = self.decoder(i_res)
        # print("res:", res.shape, res[0])
        # torch.Size([32, 1])
...
class EnhancedEmbedding(nn.Module):
    ...
    def forward(self, input_id):
        ...
        position_id = torch.arange(0,seq_len, dtype=torch.int64).to(input_id.device)
        print("input_id:", input_id.shape, input_id[0])
        print("seq_len:", seq_len)
        print("position_id:", position_id)
        position_id = position_id.unsqueeze(0).expand_as(input_id)
        print("position_id:", position_id.shape, position_id)

        word_embeddings = self.word_embedding(input_id)
        print("word_embeddings:", word_embeddings.shape, word_embeddings)
...
class SelfAttention(nn.Module):
    ...
    def score_transpose(self, x):
        ...
        # 32 50 12 32
        return x.permute(0, 2, 1, 3) # 32 12 50 32
...
class Cross_Attention(nn.Module):
    def __init__(self, hidden_size, num_attention_heads, attention_dropout_ratio, hidden_dropout_ratio, mode):
        super(Cross_Attention, self).__init__()
        
        assert mode in ['T2D', 'D2T']
        self.mode = mode
        if mode == 'T2D':
            self.crs_attn = T2D_Attention(hidden_size, num_attention_heads, attention_dropout_ratio)
        else:
            self.crs_attn = D2T_Attention(hidden_size, num_attention_heads, attention_dropout_ratio)

        self.output = SelfOutput(hidden_size, hidden_dropout_ratio)

    def forward(self, drug_hidden_states, target_hidden_states, attention_mask):
        
        crs_output = self.crs_attn(drug_hidden_states, target_hidden_states, attention_mask)
        
        if self.mode == 'T2D':
            self_output = self.output(crs_output, drug_hidden_states)
        else:
            self_output = self.output(crs_output, target_hidden_states)
        
        return self_output


class T2D_Attention(nn.Module):
    """
    Self-Attention
    """
    def __init__(self, hidden_size, num_attention_heads, attention_dropout_ratio):
        ...
        Initialization
        """
        super(T2D_Attention, self).__init__()
        if hidden_size % num_attention_heads != 0:
            raise ValueError(
                 "The hidden size (%d) is not a product of the number of attention heads (%d)" % 
                 (hidden_size, num_attention_heads))
    ...
    def forward(self, drug_hidden_states, target_hidden_states, attention_mask):
        ...
        Self-Attention block
        """
        temp_q = self.q(drug_hidden_states)
        temp_k = self.k(target_hidden_states)
        temp_v = self.v(target_hidden_states)
...
class D2T_Attention(nn.Module):
    ...
    def __init__(self, hidden_size, num_attention_heads, attention_dropout_ratio):
        ...
        Initialization
        """
        super(D2T_Attention, self).__init__()
        if hidden_size % num_attention_heads != 0:
            raise ValueError(
                 "The hidden size (%d) is not a product of the number of attention heads (%d)" % 
                 (hidden_size, num_attention_heads))
    ...
    def forward(self, drug_hidden_states, target_hidden_states, attention_mask):
        ...
        Self-Attention block
        """
        temp_q = self.q(target_hidden_states)
        temp_k = self.k(drug_hidden_states)
        temp_v = self.v(drug_hidden_states)
