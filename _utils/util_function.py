...
from data.data import drug_encoder, target_encoder
...
def load_davis_dataset():
    ...
    trainn_fold = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'DAVIS', 'folds', 'train_fold_setting1.txt')))
    train_fold = []
    ...
    test_fold = json.load(
        open(os.path.join('./', 'dataset', 'regression', 'benchmark', 'DAVIS', 'folds', 'test_fold_setting1.txt')))
    ligands = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'DAVIS', 'ligands_can.txt')),
        object_pairs_hook=OrderedDict)
    proteins = json.load(
        open(os.path.join('./', 'dataset', 'regression', 'benchmark', 'DAVIS', 'proteins.txt')),
        object_pairs_hook=OrderedDict)
    
    affinity = pickle.load(open(os.path.join('./','dataset', 'regression', 'benchmark', 'DAVIS', 'Y'),
                                'rb'), encoding='latin1')
    ...
    os.makedirs(os.path.join('./','dataset', 'regression', 'benchmark', 'DAVIS', 'processed'), exist_ok=True)
    train_test_dataset = []
    for split in ['train', 'test']:
        split_dir = os.path.join('./','dataset', 'regression', 'benchmark', 'DAVIS', 'processed', split)
...
def load_kiba_dataset():
    ...
    """
    trainn_fold = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'folds', 'train_fold_setting1.txt')))
    train_fold = []
    for e in zip(*trainn_fold):
        for ee in e:
    ...
    # train_fold = [ee for e in trainn_fold for ee in e]
    test_fold = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'folds', 'test_fold_setting1.txt')))
    ligands = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'ligands_can.txt')),
        object_pairs_hook=OrderedDict)
    proteins = json.load(
        open(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'proteins.txt')),
        object_pairs_hook=OrderedDict)
    
    affinity = pickle.load(open(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'Y'),
                                'rb'), encoding='latin1')
    smiles_lst, protein_lst = [], []
    ...
    os.makedirs(os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'processed'), exist_ok=True)
    train_test_dataset = []
    for split in ['train', 'test']:
        split_dir = os.path.join('./','dataset', 'regression', 'benchmark', 'KIBA', 'processed', split)
        os.makedirs(split_dir, exist_ok=True)
        fold = train_fold if split == 'train' else test_fold
        rows, cols = np.where(np.isnan(affinity) == False)
        rows, cols = rows[fold], cols[fold]
