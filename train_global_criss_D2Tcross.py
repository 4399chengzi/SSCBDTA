...
def reg_test(data_generator, model):
    ...
    for _, data in enumerate(data_generator):
        d_out, mask_d_out, t_out, mask_t_out, label, label_drugSeqs, label_targetSeqs = \
            data['d_out'], data['mask_d_out'], data['t_out'], data['mask_t_out'], data['label'], data['label_drugSeqs'], data['label_targetSeqs']
        ...
        temp = model(torch.as_tensor(d_out).long().cuda(), torch.as_tensor(t_out).long().cuda(), \
                    torch.as_tensor(mask_d_out).long().cuda(), torch.as_tensor(mask_t_out).long().cuda(),
...
def main(args):
    ...
    reg_train_loader = DataLoader(reg_training_data, batch_size=args.batchsize, 
                                    shuffle=True, drop_last=False, num_workers=args.workers, collate_fn=DistCollateFn())
    ...
    reg_validation_loader = DataLoader(reg_validation_data, batch_size=args.batchsize, 
                                    shuffle=False, drop_last=False, num_workers=args.workers,collate_fn=DistCollateFn())
    ...
    for epoch in range(args.epochs):
        ...
        for batch_id, data in enumerate(reg_train_loader):
            ...
            pad_drug, drug_mask = encode(label_drugSeqs, -1) # padding and obtain pad mask
            pad_tar, tar_mask = encode(label_targetSeqs, -1) # padding and obtain pad mask
            # print("d_out:",len(d_out),len(d_out[0]),d_out[0])
            # print("mask_d_out:",len(mask_d_out),len(mask_d_out[0]),mask_d_out[0])
            # print("t_out:",len(t_out),len(t_out[0]),t_out[0])
            # print("mask_t_out:",len(mask_t_out),len(mask_t_out[0]),mask_t_out[0])
            # print("label:",len(label),label[0])
            # print("label_drugSeqs:",len(label_drugSeqs),len(label_drugSeqs[0]),label_drugSeqs[0])
            # print("label_targetSeqs:",len(label_targetSeqs),len(label_targetSeqs[0]),label_targetSeqs[0])
            # print("pad_drug:", len(pad_drug), len(pad_drug[0]),pad_drug[0])
            # print("drug_mask:", len(drug_mask), len(drug_mask[0]),drug_mask[0])
            # print("pad_tar:", len(pad_tar), len(pad_tar[0]),pad_tar[0])
            # print("tar_mask:", len(tar_mask), len(tar_mask[0]),tar_mask[0])
        ...
        with torch.no_grad():
            ...
            if mse < optimal_mse:
                ...
                torch.save(model.state_dict(), os.path.join('ckpts_global_criss_D2Tcross_kiba',f'{args.dataset}_bestMSE_model_reg1_before_step32_kiba'))
            if CI > optimal_CI:
                optimal_CI = CI
                print("Saving the best_model with best CI...")
                print("Best CI: {}".format(optimal_CI))
                torch.save(model.state_dict(), os.path.join('ckpts_global_criss_D2Tcross_kiba',f'{args.dataset}_bestCI_model_reg1_before_step32_kiba'))
            if RM2 > optimal_RM2:
                optimal_RM2 = RM2
                print("Saving the best_model with best RM2...")
                print("Best RM2: {}".format(optimal_RM2))
                torch.save(model.state_dict(), os.path.join('ckpts_global_criss_D2Tcross_kiba',f'{args.dataset}_bestRM2_model_reg1_before_step32_kiba'))
